        ...
        <div class="exercises">
          ...
          <ul>
            <li v-for="exercise in filteredExercises" :key="exercise.id" class="exercise-item">
              ...
              <textarea v-model="descriptionMap[selectedWeek][exercise.id]" @change="updateDescription(exercise.id, selectedWeek)"></textarea>
              <button @click="updateDescription(exercise.id, selectedWeek)">Update</button>
            </li>
          </ul>
        </div>
      </div>
...
export default defineComponent({
  setup() {
    ...
    const descriptionMap = ref<Record<number, Record<number, string>>>({});
    ...
    const fetchProgramDetails = async () => {
      ...
      try {
        ...
        loading.value = false;
        // Initialize descriptionMap
        if (program.value) {
          program.value.exercises.forEach(exercise => {
            exercise.evolutions.forEach(evolution => {
              if (!descriptionMap.value[evolution.week_number]) {
                descriptionMap.value[evolution.week_number] = {};
              }
              descriptionMap.value[evolution.week_number][exercise.id] = evolution.description;
            });
          });
        }
      } catch (error) {
    ...
    const updateDescription = async (exerciseId: number, weekNumber: number) => {
      const description = descriptionMap.value[weekNumber][exerciseId];
      const exercise = program.value?.exercises.find(ex => ex.id === exerciseId);
      const evolutionId = exercise?.evolutions.find(evo => evo.week_number === weekNumber)?.id;
      if (!description || !evolutionId) {
        return;
      }

      try {
        await apiClient.patch(`http://localhost:8000/api/evolution/${evolutionId}/`, {
